...
void print_usage(FILE* stream) {
	...
	fprintf(stream, "  -o | --output <filename>: Output to <filename> after processing\n");
...
int main(int argc, char** argv) {
	...
	if (!apply_seccomp_filter(input_fd, output_fd)) {
		...
		eprintf("WARNING: Failed to apply seccomp filter.. (-DALLOW_SECCOMP_FAIL)\n");
		#else
		eprintf("FATAL: Failed to apply seccomp filter.. (-DALLOW_SECCOMP_FAIL not set)\n");
		goto error;
		#endif
	}
	...
	switch (mode) {
		...
		case DECRYPT:
				...
				&& fread(&memlimit, sizeof(memlimit), 1, fp_input) == 1)
			{
				...
				// Valid header, opslimit and memlimit have already been set, nothing to do
			}
			else {
		...
		case UNSPECIFIED:
			...
			{
				if (opslimit == 0) {
					eprintf("WARNING: %s: Contains nacrypt header but OPSLIMIT is invalid, treating as encrypt..\n", input_filename);
				...
				if (memlimit == 0) {
					eprintf("WARNING: %s: Contains nacrypt header but MEMLIMIT is invalid, treating as encrypt..\n", input_filename);
					opslimit = NACRYPT_OPSLIMIT_DEFAULT;
					memlimit = NACRYPT_MEMLIMIT_DEFAULT;
					rewind(fp_input); // Go back to the start
					mode = ENCRYPT;
	...
	// Ask for password
	if (mode == ENCRYPT) printf("Please create a password: ");
	else printf("Please enter password: ");
	fflush(stdout);
	...
	if (mode == ENCRYPT) {
		...
		if (!encrypt_file(fp_input, fp_output, password, (unsigned long long)opslimit, (size_t)memlimit)) goto error;
	}
	else {
		if (!decrypt_file(fp_input, fp_output, password, (unsigned long long)opslimit, (size_t)memlimit)) goto error;
	}
